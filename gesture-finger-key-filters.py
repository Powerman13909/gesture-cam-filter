...
if not cap.isOpened():
    print("Erro could not access cam")
    exit()
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
             ...
             thumb_tip = hand_landmarks.landmark[mp_hands.HandLandmarks.THUMB_TIP]
             index_tip = hand_landmarks.landmark[mp_hands.HandLandmarks.INDEX_FINGER_TIP]
             middle_tip = hand_landmarks.landmark[mp_hands.HandLandmarks.MIDDLE_FINGER_TIP]
             ring_tip = hand_landmarks.landmark[mp_hands.HandLandmarks.RING_TIP]
             ...
             if abs(thumb_x - index_x) < 30 and abs (thumb_y - index_y) < 30:
                 if current_time -last_action_time > debouce_time:
